import { FeatureCard } from "./featureCard";
import dabur from "../assets/dabur.png";
import tata1mg from "../assets/tata_1mg.png"
import baidynath from "../assets/Baidynath.png"
import himalaya from "../assets/himalaya.png"
import jiva from "../assets/jiva.png"
import sri from "../assets/sri.png"
export const AyurvedaBrands = () => {
  return (
    <>
      <div className="flex justify-between pe-3">
        <span className="text-lg ps-10 ">Ayurveda top brands</span>
        <button className="bg-red-500 text-white font-semibold text-xs rounded-sm px-2 py-1">
          SEE ALL
        </button>
      </div>
      <div className="grid grid-flow-col gap-x-3 items-center w-full h-60 bg-white shadow-card my-5 px-12 overflow-x-scroll hideScroll mb-5">
        <FeatureCard logo={dabur} />
        <FeatureCard logo={tata1mg} />
        <FeatureCard logo={baidynath} />
        <FeatureCard logo={himalaya} />
        <FeatureCard logo={jiva} />
        <FeatureCard logo={sri} />
        <FeatureCard logo={dabur} />
        <FeatureCard logo={tata1mg} />
        <FeatureCard logo={dabur} />
      </div>
    </>
  );
};
